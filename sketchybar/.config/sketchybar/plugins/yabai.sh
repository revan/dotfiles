...
windows_on_spaces() {
  ...
  app_index=0
  apps="$(yabai -m query --windows | jq -r ".[] | select(.\"is-visible\") | .id | @sh")"
  ...
  if [ "$apps" != "" ]; then
    ...
    while IFS= read -r app_id; do
      ...
      app=$(yabai -m query --windows --window "$app_id" | jq -r ".app")
      title=$(yabai -m query --windows --window "$app_id" | jq -r ".title" | sed 's/\(.\{30\}\).*/\1…/')
      args+=(--set "app.$app_index" label="$title ($app)" drawing=on)
    done <<< "$apps"

    sketchybar -m "${args[@]}"
  fi

  while [[ app_index -lt 15 ]]; do
    app_index=$((app_index + 1))
    sketchybar --set "app.$app_index" drawing=off
  done
}
